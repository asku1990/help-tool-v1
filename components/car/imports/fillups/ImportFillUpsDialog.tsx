...
function toBoolOptional(v?: string): boolean | undefined {
...
function parseFillUpsCsv(input: string): ParsedRow[] {
  const lines = input
    .split(/\r?\n/)
    .map(l => l.trim())
    .filter(l => l.length > 0);
  ...
  if (hasHeader) {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const isFull = toBoolOptional(idx.isFull >= 0 ? tokens[idx.isFull] : undefined);
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const isFull = toBoolOptional(tokens[5]);
